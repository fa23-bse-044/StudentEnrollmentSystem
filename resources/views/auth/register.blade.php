...
<head>
    ...
    <title>Create Account | COMSATS University</title>
    <style>
...
<body>
    ...
    <div class="auth-container">
        <div class="logo-header">
            ...
            <p style="margin: 0; color: #64748b; font-size: 14px;">Create Academic Account</p>
        </div>
...
<div id="student_reg_field" style="display:none;">
    <input type="text" name="reg_no" placeholder="Registration Number (e.g., FA23-BSE-044)">
</div>

<script>
    document.querySelector('select[name="role"]').addEventListener('change', function() {
        document.getElementById('student_reg_field').style.display = (this.value === 'student') ? 'block' : 'none';
    });
    // Trigger initial on-load state checking
    window.addEventListener('DOMContentLoaded', () => {
        document.getElementById('student_reg_field').style.display = (document.querySelector('select[name="role"]').value === 'student') ? 'block' : 'none';
    });
</script>
        ...
        <p style="text-align: center; font-size: 14px; color: #64748b; margin-top: 20px;">
            Already have an account? <a href="{{ route('login') }}" style="color: #00a896; font-weight: bold; text-decoration: none;">Login</a>
        </p>
    </div>
